...
def _issue_comment_event(*, pull_request: bool = False, body: str = "Looks good to me.") -> dict:
    issue: dict[str, object] = {
        ...
        "html_url": "https://github.com/Tracer-Cloud/opensore/issues/42",
    ...
    return {
        ...
        "issue": issue,
        "comment": {
            "html_url": "https://github.com/Tracer-Cloud/opensore/issues/42#issuecomment-1",
            "body": body,
...
def test_notification_from_issue_comment_event_extracts_fields() -> None:
    notification = notification_from_issue_comment_event(
        ...
        repository="Tracer-Cloud/opensore",
    )

    assert notification is not None
    assert notification.repository == "Tracer-Cloud/opensore"
...
def test_build_slack_payload_truncates_long_comment_preview() -> None:
    ...
    assert "New comment on Tracer-Cloud/opensore#42" in payload["text"]
...
def test_main_posts_notification(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("GITHUB_REPOSITORY", "Tracer-Cloud/opensore")
