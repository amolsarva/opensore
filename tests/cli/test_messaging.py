...
class TestMessagingPairCommand:
    ...
    def test_pair_supports_whatsapp(self, _isolated_store: Path) -> None:
        runner = CliRunner()
        result = runner.invoke(messaging, ["pair", "--platform", "whatsapp"])
        assert result.exit_code == 0

        data = json.loads(_isolated_store.read_text())
        assert data["integrations"][0]["service"] == "whatsapp"

    def test_pair_supports_imessage(self, _isolated_store: Path) -> None:
        runner = CliRunner()
        result = runner.invoke(messaging, ["pair", "--platform", "imessage"])
        assert result.exit_code == 0

        data = json.loads(_isolated_store.read_text())
        assert data["integrations"][0]["service"] == "imessage"
