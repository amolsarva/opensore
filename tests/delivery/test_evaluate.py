...
def _make_state(*, evaluate: bool = False, rubric: str = "") -> dict[str, Any]:
    ...
    state = make_initial_state(raw_alert=raw, opensore_evaluate=evaluate)
    return dict(state)
...
def test_deliver_runs_judge_when_evaluate_and_rubric_present(
    ...
    monkeypatch.setattr(
        "app.integrations.opensore.llm_eval_judge.run_opensore_llm_judge",
        mock_judge,
    )

    state = _make_state(evaluate=True, rubric="test rubric")
    deliver(state)
    assert state["opensore_llm_eval"] == fake
...
def test_deliver_skips_judge_when_evaluate_is_false(
    ...
    state["opensore_eval_rubric"] = "test rubric"
    deliver(state)
    assert not state.get("opensore_llm_eval")
