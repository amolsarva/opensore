...
**Workplace discovery direction** - OpenSRE is being adapted for workplace
misconduct discovery: harassment, executive misconduct, retaliation, and sexual
misconduct investigations across workplace systems. The hosted mode is designed
to avoid storing user evidence on the OpenSRE host; users authenticate their own
source accounts and exports are written as CSV files to user-owned Google Drive.
...
See `docs/workplace-discovery-product-plan.mdx` for the top-down plan.

**Mac personal-agent path** - if you want OpenSRE to become a local OpenClaw-like assistant for cheap LLMs plus WhatsApp/iMessage workflows, start with:

```bash
opensre personal doctor
opensre integrations setup whatsapp
opensre messaging pair --platform whatsapp
```

See `docs/macos-personal-agent-quickstart.mdx` and `docs/personal-agent-roadmap.mdx` for the current gap list. WhatsApp is delivery-only today; iMessage needs an OpenClaw or local bridge until native support is built.
