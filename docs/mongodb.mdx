---
title: "MongoDB"
description: "Connect MongoDB so OpenSore can diagnose database issues during investigations"
---

OpenSore uses MongoDB diagnostics to investigate database-related alerts — checking server health, finding slow queries, monitoring replica sets, and analyzing collection statistics.

## Prerequisites

- MongoDB 4.0+ (4.4+ recommended)
- Network access from the OpenSore environment to your MongoDB instance
...
When OpenSore investigates a MongoDB-related alert, five diagnostic tools are available:
...
| Symptom | Fix |
| --- | --- |
| **Connection refused** | Verify the host/port, check firewalls, and ensure MongoDB is running. For Atlas, whitelist the OpenSore IP. |
