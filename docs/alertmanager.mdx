---
title: "Alertmanager"
description: "Connect Alertmanager so OpenSore can surface firing alerts and active silences during investigations"
---

OpenSore queries Alertmanager to retrieve firing, silenced, and inhibited alerts — correlating the triggering alert with concurrent signals to narrow root-cause hypotheses faster.

## Prerequisites

- Alertmanager v0.20+ reachable from the machine running OpenSore
...
```bash
opensore integrations setup alertmanager
```
...
```bash
opensore integrations verify alertmanager
```
...
When an Alertmanager integration is configured, OpenSore automatically includes it as an evidence source during every investigation. Two tools become available to the investigation agent:
...
| Symptom | Fix |
| --- | --- |
| **Status: missing** | Set `ALERTMANAGER_URL` or run `opensore integrations setup alertmanager` |
...
- Use a **read-only** reverse-proxy token when possible — OpenSore only reads alerts and silences and never writes to Alertmanager during investigations.
