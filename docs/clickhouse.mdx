---
title: "ClickHouse"
description: "Connect ClickHouse so OpenSore can inspect query activity and system health during investigations"
---

OpenSore queries ClickHouse system tables to surface slow queries, active connections, replica status, and table statistics — helping diagnose database performance issues during alert investigations.

## Prerequisites

- ClickHouse 21.x or later
- Network access from the OpenSore environment to your ClickHouse instance
...
GRANT SELECT ON system.* TO opensore_readonly;
GRANT SELECT ON default.* TO opensore_readonly;
```

## Investigation tools

When OpenSore investigates a ClickHouse-related alert, three diagnostic tools are available:
...
```bash
opensore integrations verify clickhouse
```
...
| **Access denied on system tables** | Grant `SELECT ON system.*` to the OpenSore user |

## Security best practices

- Create a **dedicated read-only user** with access only to `system.*` and the databases OpenSore needs.
