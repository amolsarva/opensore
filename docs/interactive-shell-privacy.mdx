...
description: 'How OpenSore redacts secrets from your command history, and the controls you have over persistence'
---

## Overview

The OpenSore interactive shell persists every line you type to a history file so up-arrow recall and `/history` work across sessions. Incident prompts can include sensitive identifiers and tokens, so the shell:
...
The history file lives at `~/.config/opensore/interactive_history`.
...
Redaction applies only to **persistent history**. The line you typed is still passed to OpenSore's normal pipeline as you typed it.
...
2. The `interactive.history` block in `~/.config/opensore/config.yml`
3. Built-in defaults
...
| --- | --- | --- |
| `OPENSORE_HISTORY_ENABLED` | `1` | Set to `0`/`false`/`off` to skip persistence entirely (in-memory only). |
| `OPENSORE_HISTORY_REDACT` | `1` | Set to `0`/`false`/`off` to disable redaction (raw `FileHistory`). |
| `OPENSORE_HISTORY_MAX_ENTRIES` | `5000` | Non-negative integer. `0` disables the cap (unlimited). |
...
The history file is **plain text on local disk** at `~/.config/opensore/interactive_history`, with the user's default file permissions. Built-in redaction targets common token shapes only — it is not a substitute for proper secret handling. Treat the file as confidential and be aware:
...
For the strongest posture: set `OPENSORE_HISTORY_ENABLED=0`, accept the loss of cross-session up-arrow recall, and rely on the in-memory ring instead.
