---
title: "MySQL"
description: "Connect MySQL so OpenSore can diagnose database issues during investigations"
---

OpenSore uses MySQL diagnostics to investigate database-related alerts — checking server health, surfacing slow queries, monitoring replication status, and analyzing table statistics.

## Prerequisites

- MySQL 5.7+ (8.0+ recommended for full `performance_schema` support)
- Network access from the OpenSore environment to your MySQL instance
...
MYSQL_USERNAME=opensore_readonly
...
```sql
CREATE USER 'opensore_readonly'@'%' IDENTIFIED BY 'secure-password';

-- Required for server status and process list
GRANT PROCESS ON *.* TO 'opensore_readonly'@'%';

-- Required for table statistics
GRANT SELECT ON information_schema.* TO 'opensore_readonly'@'%';

-- Required for slow query analysis
GRANT SELECT ON performance_schema.* TO 'opensore_readonly'@'%';
...
When OpenSore investigates a MySQL-related alert, five diagnostic tools are available:
...
```bash
opensore integrations verify mysql
```
...
| **Access denied on performance_schema** | Grant `SELECT ON performance_schema.*` to the OpenSore user. |
