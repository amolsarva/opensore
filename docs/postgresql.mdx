---
title: "PostgreSQL"
description: "Connect PostgreSQL so OpenSore can diagnose database issues during investigations"
---

OpenSore uses PostgreSQL diagnostics to investigate database-related alerts — checking server health, surfacing slow queries, monitoring replication status, and analyzing table statistics.

## Prerequisites

- PostgreSQL 10+ (12+ recommended for full `pg_stat_statements` support)
- Network access from the OpenSore environment to your PostgreSQL instance
...
POSTGRESQL_USERNAME=opensore_readonly
...
Integrations are automatically persisted to `~/.config/opensore/integrations.json`:
...
```sql
-- Create the user
CREATE USER opensore_readonly WITH PASSWORD 'secure-password';

-- Grant access to system views
GRANT pg_monitor TO opensore_readonly;

-- Grant access to the target database
GRANT CONNECT ON DATABASE application_db TO opensore_readonly;
\c application_db
GRANT USAGE ON SCHEMA public TO opensore_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO opensore_readonly;
```
...
When OpenSore investigates a PostgreSQL-related alert, five diagnostic tools are available:
...
<Info>
Slow query data requires the `pg_stat_statements` extension to be installed and loaded via `shared_preload_libraries`. OpenSore returns an informative message if the extension is not available.
</Info>
...
```bash
opensore integrations verify postgresql
```
...
| **Permission denied on pg_stat_activity** | Grant `pg_monitor` role to the OpenSore user. |
