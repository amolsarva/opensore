---
title: "Azure Monitor"
description: "Connect Azure Monitor Log Analytics so OpenSore can pull KQL log evidence during investigations"
---

OpenSore queries Azure Monitor Log Analytics through the public Query REST API to surface relevant logs during alert investigations. Each query is bounded by a `take` clause so result sets stay capped at a safe row limit.
...
- Network access from the OpenSore environment to `https://api.loganalytics.io` (or the sovereign cloud equivalent) over HTTPS
...
| `AZURE_MAX_RESULTS` | `100` | Per-query row cap; OpenSore clamps to a hard maximum of `200` |

### Option 2: Persistent store

Credentials are persisted to `~/.config/opensore/integrations.json` with `0o600` permissions:
...
2. Give the app a name (e.g. `opensore-log-analytics`) and register it as a single-tenant app.
...
OpenSore exposes one tool against an Azure Monitor workspace:
...
- **`query`** — KQL query text. If omitted, OpenSore falls back to `AppTraces | order by TimeGenerated desc | take <limit>`.
...
OpenSore always appends a `| take <limit>` clause to the query if one is not present, so the workspace never returns more rows than the configured cap.

## Verify

```bash
opensore integrations verify azure
```
...
The verify step is a credential-shape check — it does not call the workspace. To exercise the live path, point OpenSore at a synthetic alert that names `azure` as the source and inspect the resulting evidence.
...
- Use a **dedicated app registration** for OpenSore — do not reuse a personal token or a broadly-scoped service principal.
- Grant only **Log Analytics Reader** on the workspace; OpenSore only needs read access to query data.
...
- The integration is **read-only**: OpenSore only issues `POST /v1/workspaces/<id>/query` requests with a `take`-bounded KQL string.
