---
title: "Kafka"
description: "Connect Kafka so OpenSore can inspect topic health and consumer group lag during investigations"
---

OpenSore queries Kafka to retrieve topic partition health, consumer group lag, and broker metadata — helping diagnose lag spikes, under-replicated partitions, and consumer group failures during incidents.

## Prerequisites

- Apache Kafka cluster (2.x or later)
- Network access from the OpenSore environment to the Kafka brokers
...
When OpenSore investigates a Kafka-related alert, two diagnostic tools are available:
...
```bash
opensore integrations verify kafka
```
