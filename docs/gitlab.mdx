...
OpenSore's GitLab integration gives the investigation agent read access to your merge requests, commits, pipelines, and files for root cause analysis. Optionally, it can write investigation findings back as a note on the relevant MR.

---

## Step 1: Create a GitLab Access Token

OpenSore supports both **Personal Access Tokens** and **Project Access Tokens**. A Project Access Token is recommended for production — it is scoped to a single project and does not depend on a user account.

### Personal Access Token (quickest for local use)

1. In GitLab, go to your avatar → **Edit profile** → **Access Tokens**.
2. Click **Add new token**.
3. Give it a name (e.g. `opensore`) and set an expiry date.
...
<Note>
If your GitLab instance is self-hosted, make sure your OpenSore server can reach it over the network before proceeding.
</Note>

---

## Step 2: Configure the Integration in OpenSore
...
Once connected, OpenSore automatically uses GitLab as an investigation source when an alert contains a GitLab MR reference. The agent can:
...
OpenSore can post a formatted investigation summary directly as a note on the GitLab MR that triggered the alert. This is opt-in and disabled by default.
...
When enabled, after each investigation OpenSore posts a collapsible `### RCA Finding` note on the MR. The note is truncated to 4000 characters if needed.

**Requirements for write-back:**
- Your access token must have the `api` scope (not just `read_api`)
- The alert payload must include a GitLab MR IID and project ID so OpenSore can identify the target MR
...
Your token is invalid or has expired. Regenerate it in GitLab and re-run `opensore onboard`.
...
Verify that `GITLAB_BASE_URL` ends in `/api/v4` and that your OpenSore server can reach the GitLab host. Test with:
...
Check that `GITLAB_MR_WRITEBACK=true` is set in your environment and that the alert payload contains `gitlab.merge_request_iid` and `gitlab.project_id` fields. Review OpenSore server logs for `[publish] GitLab MR` entries.
