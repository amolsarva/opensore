...
description: "Connect Better Stack so OpenSore can pull log evidence from your Telemetry sources during investigations"
---

OpenSore uses Better Stack's ClickHouse SQL Query API to read log evidence during investigations. It queries the configured source via `remote(<source>_logs)` for recent rows and `s3Cluster(primary, <source>_s3)` for historical rows, bounded by the alert window.
...
- Network access from the OpenSore environment to that endpoint over HTTPS
...
```bash
opensore integrations setup betterstack
```
...
1. Open **Telemetry** and pick the source you want OpenSore to query.
...
4. The **base source ID** (e.g. `t123456_myapp`) is shown above the integration panel and is the value to supply for `BETTERSTACK_SOURCES`. Use the base name only — OpenSore appends `_logs` and `_s3` internally.

## Investigation tool

OpenSore exposes one tool against a Better Stack source:
...
```bash
opensore integrations verify betterstack
```
...
- Use a **dedicated ClickHouse HTTP client credential** for OpenSore — not your personal dashboard login.
- Keep the credential pair out of source control — use `.env` or the persistent store (`~/.config/opensore/integrations.json`).
- The integration is **read-only**: OpenSore only issues `SELECT` statements against `remote(...)` and `s3Cluster(...)` table functions.
