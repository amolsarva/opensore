---
title: "AWS"
description: "Connect AWS so OpenSore can map your infrastructure and investigate cloud-related alerts"
---

OpenSore uses AWS to map your environment: Lambda functions, EKS clusters, S3 buckets, and more. It reads infrastructure state to build investigation context when cloud-related alerts fire.
...
```bash
AWS_ROLE_ARN=arn:aws:iam::123456789012:role/OpenSoreReadOnly
...
OpenSore requires read-only access. Attach the following managed policies to the IAM role or user:

- `ReadOnlyAccess` (AWS managed) — or a custom policy scoped to the services you want OpenSore to inspect
...
```bash
opensore integrations verify aws
```
...
Service: aws
Status: passed
Detail: Authenticated via assume-role in us-east-1 as arn:aws:iam::123456789012:role/OpenSoreReadOnly (account 123456789012)
```
...
- Scope IAM permissions to only the AWS services OpenSore needs to inspect.
- Rotate static access keys regularly.
- Enable CloudTrail so all OpenSore API calls are auditable.
