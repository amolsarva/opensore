---
title: "Splunk"
description: "Connect Splunk so OpenSore can search logs using SPL during investigations"
---

OpenSore queries Splunk using the REST API to surface relevant log evidence during alert investigations — searching indexes with SPL, correlating error patterns with incidents, and identifying root causes.
...
OpenSore uses bearer tokens — not basic auth and not HEC tokens. To generate one:

**Via the Splunk UI:**

1. Go to **Settings** → **Tokens**
2. Click **New Token**
3. Set a name (e.g. `opensore`) and an expiry date
...
```bash
opensore integrations verify splunk
```
...
OpenSore builds SPL queries deterministically from the alert payload — the LLM selects which tool to call but never writes the query itself. This keeps investigations reproducible and auditable.
...
```bash
opensore investigate --template splunk
```
...
| `opensore integrations verify` fails | Check `SPLUNK_URL` includes the protocol and port (`https://host:8089`) |
...
- Prefer a dedicated `opensore` service account with only the `search` capability.
