---
title: "MariaDB"
description: "Connect MariaDB so OpenSore can diagnose database issues during investigations"
---

OpenSore uses MariaDB diagnostics to investigate database-related alerts — checking server health, finding slow queries, monitoring replication, and analyzing active threads and InnoDB engine state.

## Prerequisites

- MariaDB 10.5+ (10.11 LTS or 11.x recommended)
- Network access from the OpenSore environment to your MariaDB instance
...
```bash
opensore integrations setup mariadb
```
...
Create a dedicated read-only user for OpenSore so it cannot modify data:

```sql
CREATE USER 'opensore_ro'@'%' IDENTIFIED BY 'strong-password';
GRANT SELECT, PROCESS, REPLICATION CLIENT ON *.* TO 'opensore_ro'@'%';
GRANT SELECT ON performance_schema.* TO 'opensore_ro'@'%';
FLUSH PRIVILEGES;
```

The `PROCESS` privilege lets OpenSore read `information_schema.PROCESSLIST`. `REPLICATION CLIENT` enables `SHOW ALL SLAVES STATUS` / `SHOW SLAVE STATUS`. `SELECT` on `performance_schema` is only needed if you want slow-query insights.
...
When OpenSore investigates a MariaDB-related alert, five diagnostic tools are available:
...
```bash
opensore integrations verify mariadb
```
...
| **Access denied for user** | Confirm the username/password and that the user is granted access from the OpenSore host (`'opensore_ro'@'%'` or a specific IP). |
