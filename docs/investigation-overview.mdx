...
Use this workflow when running OpenSore via the local `opensore` binary.

You can work in two ways:

- **Interactive prompt shell** — run `opensore` with no subcommand (TTY) to enter the REPL: describe incidents conversationally, stream investigations, and use slash commands.
- **Direct investigation** — run `opensore investigate` from your terminal with `-i` pointing at an alert payload (or `--interactive` to pick a file in the UI). The process runs and exits when the investigation completes.

## 1) Start an investigation

### Interactive shell (`opensore`)

From a terminal with stdin and stdout attached (`opensore` detects a TTY), run:
...
```bash
opensore investigate -i <alert-file> --output ./rca.json
```

## 3) Understand what OpenSore analyzed
...
If you've configured the Slack integration, OpenSore can publish a concise incident summary into Slack after the
