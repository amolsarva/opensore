...
description: 'Run opensore+LLM against the 452-scenario Cloud-OpsBench corpus (Wang et al, arXiv:2603.00468v1) and compare to published LLM-alone baselines.'
---
...
OpenSore wraps this corpus through a small reusable benchmark framework
...
is to publish the opensore+LLM column against the paper's LLM-alone
baselines on the same scenarios.

```text
Paper baseline                 opensore+LLM (this benchmark)
─────────────                  ──────────────────────────
...
# 2. Benchmark-dedicated LLM API keys — keep separate from production opensore keys
...
modes:
  - opensore+llm          # opensore wrapping the LLM
...
| Variable | Purpose |
|---|---|
| `OPENSORE_BENCH_WORKERS` | Override `workers:` |
| `OPENSORE_BENCH_COST_BUDGET_USD` | Override `cost_budget_usd:` |
...
The framework registers a usage hook on `app/services/llm_client.py`'s
...
`make test-cloudopsbench` and `opensore tests cloudopsbench` route through
...
- Framework source: [`tests/benchmarks/_framework/`](https://github.com/Tracer-Cloud/opensore/tree/main/tests/benchmarks/_framework)
- Adapter source: [`tests/benchmarks/cloudopsbench/`](https://github.com/Tracer-Cloud/opensore/tree/main/tests/benchmarks/cloudopsbench)
