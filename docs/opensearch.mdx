...
description: "Connect OpenSearch or Elasticsearch so OpenSore can search application logs and analytics indices during investigations"
---

OpenSore queries OpenSearch (or Elasticsearch) to retrieve application logs, error events, and analytics records — pulling concrete log lines into the investigation alongside metrics and traces from your other observability tools.

## Prerequisites

- OpenSearch 1.x/2.x or Elasticsearch 7.x/8.x reachable from the machine running OpenSore
...
```bash
opensore integrations setup opensearch
```
...
```bash
opensore integrations verify opensearch
```
...
When an OpenSearch integration is configured, OpenSore automatically includes it as an evidence source during every investigation. Two tools become available to the investigation agent:
...
| Symptom | Fix |
| --- | --- |
| **Status: missing** | Set `OPENSEARCH_URL` or run `opensore onboard` and pick OpenSearch |
...
- Create a **read-only** OpenSearch user for OpenSore — the agent only reads logs and analytics indices and never writes to the cluster during investigations.
- Limit the role to the indices OpenSore needs (typically logs and APM indices), not cluster-wide.
