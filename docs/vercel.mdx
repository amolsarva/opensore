---
title: "Vercel"
description: "Connect Vercel so OpenSore can check deployment status and runtime logs during investigations"
---

OpenSore queries Vercel to retrieve deployment status and runtime logs — helping identify whether a recent deployment caused an incident or surface errors from serverless function executions.
...
3. Give it a name (e.g., `opensore`) and set the scope to **Full Account** or a specific team
4. Copy the token
...
```bash
opensore integrations verify vercel
```
...
- Create a **dedicated read-only token** for OpenSore — do not reuse deployment tokens.
