...
OpenSore uses `POST /api/v5/query_range` for `query_signoz_logs`, `query_signoz_metrics`, and
...
# or: uv run opensore integrations verify signoz
```
...
SigNoz emits Prometheus-style webhook payloads. To trigger OpenSore investigations automatically:

1. In SigNoz, go to **Settings → Notification Channels**.
2. Create a **Webhook** channel pointing at your OpenSore instance:
   ```
   POST https://your-opensore-instance/investigate
   ```
...
```bash
opensore integrations setup signoz
```
...
```bash
uv run opensore investigate --input-json '{
  "alert_source": "signoz",
