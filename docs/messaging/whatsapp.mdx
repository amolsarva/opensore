...
OpenSore's WhatsApp integration delivers investigation findings to any WhatsApp number via Twilio's Messaging API.
...
```bash
opensore integrations setup whatsapp
```
...
```bash
opensore integrations verify whatsapp
```
...
### `opensore integrations verify whatsapp` errors
