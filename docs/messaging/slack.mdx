...
2. Name the app (e.g. `OpenSore`) and pick your workspace.
...
<Tabs>
  <Tab title="Direct setup command (recommended)">
    ```bash
    opensore integrations setup slack
    ```
    Paste the webhook URL when prompted. The credentials are persisted to `~/.config/opensore/integrations.json`.
  </Tab>
  <Tab title="Manual env var">
    ...
    ```
    OpenSore reads this at startup as a fallback when no Slack entry exists in `~/.config/opensore/integrations.json`. If you have previously run `opensore integrations setup slack`, remove that entry first (`opensore integrations remove slack`) or the env var will be ignored.
  </Tab>
  <Tab title="Onboarding wizard">
    ...
    <Note>
    The wizard's Slack step currently validates the URL but does not persist it. After the wizard completes, run `opensore integrations setup slack` (or set `SLACK_WEBHOOK_URL` in `.env`) so the credentials are saved.
    </Note>
  </Tab>
</Tabs>
...
```bash
opensore integrations verify slack
```
...
```bash
opensore integrations verify slack --send-slack-test
```
...
**`error: webhook_url is required.` from `opensore integrations setup slack`**
