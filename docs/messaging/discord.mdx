...
OpenSore's Discord integration lets you trigger investigations with the `/investigate` slash command and receive findings as formatted embeds in a dedicated thread — all without leaving Discord.
...
- An OpenSore host reachable from the public internet over HTTPS, so Discord can deliver interactions. For local development use a tunnel such as `ngrok`.
...
2. Give it a name (e.g. `OpenSore`) and click **Create**.
...
5. Select the server you want to add OpenSore to and click **Authorize**.
...
2. Set it to:
   ```
   https://<your-opensore-host>/discord/interactions
   ```
   Replace `<your-opensore-host>` with the public hostname where your OpenSore server is running.
3. Click **Save Changes**. Discord will immediately send a `PING` to that URL and expect a valid response — OpenSore handles this automatically.
...
## Step 4: Configure the integration in OpenSore
...
The `/investigate` command is registered globally and may take up to one hour to propagate. Re-running `opensore onboard` and selecting Discord will re-register the command.

**`DISCORD_PUBLIC_KEY not configured` error on your server**

Ensure `DISCORD_PUBLIC_KEY` is set in your environment before starting the OpenSore server. Re-run `opensore onboard` to write it to `.env`.
...
The background investigation task encountered an error, or your server did not respond to the followup within Discord's 15-minute window. Check your OpenSore server logs for details.
