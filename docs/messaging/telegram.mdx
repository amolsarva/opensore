...
OpenSore's Telegram integration delivers investigation findings to any chat your bot has been added to — useful for mobile-first on-call rotations and personal alerting.

<Note>
Unlike Slack and Discord, Telegram is **not** included in the interactive `opensore onboard` wizard today. Configure it via environment variables or by editing `~/.config/opensore/integrations.json` directly. Tracked at [#1481](https://github.com/Tracer-Cloud/opensore/issues/1481).
</Note>
...
2. Send `/newbot`.
3. When prompted, send a **display name** for your bot (e.g. `OpenSore Alerts`).
4. Send a **username** that ends in `bot` (e.g. `opensore_alerts_bot`). It must be globally unique.
...
OpenSore picks these up at startup and registers Telegram as an active integration.
...
```bash
opensore integrations verify telegram
```
...
```bash
opensore investigate --input tests/e2e/kubernetes/fixtures/datadog_k8s_alert.json
```
...
`opensore integrations verify telegram` only calls Telegram's `getMe` endpoint, so it surfaces token-validity errors but cannot detect chat-routing problems. Delivery-time errors only show up when an investigation actually posts.

### Errors from `opensore integrations verify telegram`

**`Missing bot_token`**

`TELEGRAM_BOT_TOKEN` is empty. Re-check `.env` and restart any long-running OpenSore process so it re-reads the file.
...
These are Telegram API responses that come back when OpenSore actually tries to post a finding. `verify` only calls `getMe`, so it cannot catch them. They appear in OpenSore logs as `[telegram] post message failed: <description>` with the Telegram description copied verbatim.
