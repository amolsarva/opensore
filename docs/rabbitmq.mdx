---
title: "RabbitMQ"
description: "Connect RabbitMQ so OpenSore can diagnose queue backlogs, consumer issues, and broker health during investigations"
---

OpenSore uses the RabbitMQ Management HTTP API to investigate message-bus incidents — checking queue backlogs, consumer health, broker-wide resource usage, cluster partition state, and connection patterns.
...
- The **`rabbitmq_management`** plugin must be enabled on the broker:
  ...
  ```
- Network access from the OpenSore environment to the Management API port (default **15672**)
...
```bash
opensore integrations setup rabbitmq
```
...
Credentials are automatically persisted to `~/.config/opensore/integrations.json` with `0o600` permissions:
...
Create a dedicated monitoring user so OpenSore has read-only access:

```bash
# Create user
rabbitmqctl add_user opensore_ro strong-password

# Grant the monitoring tag (read-only management API access)
rabbitmqctl set_user_tags opensore_ro monitoring

# Grant read-only permissions on the target vhost
rabbitmqctl set_permissions -p / opensore_ro "^$" "^$" ".*"
```
...
When OpenSore investigates a RabbitMQ-related alert, five diagnostic tools are available:
...
```bash
opensore integrations verify rabbitmq
```
...
| Symptom | Fix |
| --- | --- |
| **Connection refused on port 15672** | Verify the management plugin is enabled (`rabbitmq-plugins enable rabbitmq_management`) and that the port is reachable from the OpenSore host. |
...
| **Forbidden (403)** | The user lacks sufficient tags. Grant at least `monitoring`: `rabbitmqctl set_user_tags opensore_ro monitoring`. |
...
- The Management API is **read-only** from OpenSore's perspective — no messages are published, consumed, or deleted.
