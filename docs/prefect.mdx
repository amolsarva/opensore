---
title: "Prefect"
description: "Connect Prefect so OpenSore can inspect flow runs, workers, and deployments during investigations"
---

OpenSore queries Prefect to retrieve recent flow runs, their logs, worker health, and deployment status — helping diagnose pipeline failures and identify stalled or crashed orchestration jobs.
...
When OpenSore investigates a Prefect-related alert, three diagnostic tools are available:
...
- Store credentials in `~/.config/opensore/integrations.json`, not in source code.
