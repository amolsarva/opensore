...
OpenSore treats every other AI agent running on your machine — Claude Code,
...
discovered (`opensore agents scan`), the dashboard itself (`/agents`),
then live tail of agent stdout (`/agents trace`), then the cross-agent
context bus (`/agents bus`).

## `opensore agents scan` — discover running agent sessions

`opensore agents scan` enumerates running AI-coding-agent sessions visible to
...
`opensore agents scan --all` relaxes the argv requirements so that helper and
...
- **`-`**: never observed. Either the provider has no meter yet
  ...
  not running (non-interactive `opensore agents list` never starts
  the sampler).
...
OpenSore interactive shell — the equivalent of `kubectl logs -f` for the
...
The pid does **not** have to be in the OpenSore registry; if it is, the
...
- **Not for TTY/PTY targets.** Foreground processes whose stdout is
  ...
  PTY interception for OpenSore-spawned agents.
...
- **Socket**: Unix-domain stream socket at `~/.config/opensore/agents-bus.sock`.
- **PID sidecar**: `~/.config/opensore/agents-bus.sock.pid` (mode `0600`). The
...
- **Topology**: self-electing broker. The first `publish()` or `subscribe()`
  ...
  when OpenSore itself is not running.
...
Publishers without a Python dependency on OpenSore can connect directly:

```bash
python - <<'EOF'
import json, os, socket, uuid, datetime
sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
sock.connect(os.path.expanduser("~/.config/opensore/agents-bus.sock"))
