---
title: "OpenClaw"
description: "Connect OpenSore to OpenClaw for native context lookup and RCA write-back"
---

OpenSore integrates with OpenClaw in a native, one-way support loop:

- OpenSore can search recent OpenClaw conversations during an investigation.
- OpenSore can read full OpenClaw threads for extra engineering context.
- OpenSore writes completed RCA findings back into OpenClaw through the OpenClaw bridge.

OpenSore no longer ships a separate `opensore-mcp` command. You do not need to register OpenSore itself as an MCP server inside OpenClaw.

## Setup

### 0. Preflight the local OpenClaw CLI

Before you onboard OpenClaw in OpenSore, make sure the OpenClaw CLI itself is healthy in the same shell:
...
`nvm alias default 22` only affects future shells. It does not switch the current shell that launches `uv run opensore`.

### 1. Install and configure OpenSore locally
...
### 2. Configure the OpenSore → OpenClaw bridge
...
After every completed investigation, OpenSore attempts to write the full RCA report back into OpenClaw.

- If the investigation state contains an `openclaw_context.conversation_id`, OpenSore first tries to append the result to that conversation.
- Otherwise, OpenSore creates a new OpenClaw conversation containing the RCA report, root cause, remediation steps, and confidence score.

## Investigation behavior

When OpenClaw is configured, OpenSore now prefers the high-signal conversation actions first:
...
```bash
uv run opensore integrations verify openclaw
uv run opensore integrations verify grafana
uv run opensore integrations verify github
uv run opensore investigate -i tests/fixtures/openclaw_test_alert.json
```
...
- `uv run opensore integrations verify openclaw` fails against `http://127.0.0.1:18789/`: that URL is the OpenClaw Control UI/Gateway, not the MCP bridge. Use `stdio` mode with `openclaw mcp serve`.
...
The OpenClaw e2e suite lives at [`tests/e2e/openclaw/`](https://github.com/Tracer-Cloud/opensore/tree/main/tests/e2e/openclaw). See its [README](https://github.com/Tracer-Cloud/opensore/tree/main/tests/e2e/openclaw/README.md) for prerequisites, how to run the suite, scenario layout, and CI behavior.
