...
`opensore investigate --service <name>` kicks off a runtime investigation for a deployed service. Instead of passing an alert payload, OpenSore gathers live signals from the service (deployment status, recent logs, health probe) and feeds them into the existing investigation pipeline as evidence.

## Prerequisites

You must have deployed a service via `opensore deploy` (or registered a named remote) and configured a remote ops provider.

1. **Deploy or register a named remote:**
   ```bash
   opensore deploy ec2        # creates a named remote "ec2"
   # or register manually:
   opensore remote health https://my-service.up.railway.app
   ```

2. **Configure the remote ops provider (once):**
   ```bash
   opensore remote ops status     # prompts for provider/project/service on first run
   ```

## Usage

```bash
opensore investigate --service <name>
```

For example:
```bash
opensore investigate --service api-backend
opensore investigate --service api-backend --output ./rca.json
```
...
The output is the same structured RCA report you'd get from running `opensore investigate -i <alert-file>`.
...
opensore investigate --service api-backend --slack-thread C01234/1712345.000001
```
