---
title: "Jira"
description: "Connect Jira so OpenSore can create and update incident tickets automatically"
---

OpenSore connects to Jira to create incident tickets, update existing issues, and add investigation findings as comments — keeping your team's workflow in sync with automated investigations.
...
Add to `~/.config/opensore/integrations.json`:
...
3. Give it a label (e.g., `opensore`)
4. Copy the token
...
- Use a **dedicated Jira user** for OpenSore with only the permissions it needs (create and comment on issues).
- Store credentials in `~/.config/opensore/integrations.json`, not in source code or environment variables.
