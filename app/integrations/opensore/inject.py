"""Merge tracer-cloud/opensore CSV telemetry into resolved Grafana integrations."""
...
def _dataset_root(ann: dict[str, Any], raw_alert: dict[str, Any]) -> str:
    """Resolve local clone root for tracer-cloud/opensore (or OpenRCA-style exports)."""
    for key in (
        ...
        if v and str(v).strip():
            return str(v).strip()
    return (
        os.environ.get("OPENSORE_DATASET_ROOT", "").strip()
...
def _hf_dataset_id(ann: dict[str, Any], raw_alert: dict[str, Any]) -> str | None:
    ...
    for key in (
        ...
        if v and str(v).strip():
            return str(v).strip()
    env_id = os.environ.get("OPENSORE_HF_DATASET_ID", "").strip()
    return env_id or None


def _telemetry_relative(ann: dict[str, Any], raw_alert: dict[str, Any]) -> str | None:
    rel = (
        ann.get("opensore_telemetry_relative")
        or raw_alert.get("opensore_telemetry_relative")
    ...
    if os.environ.get("OPENSORE_INFER_TELEMETRY", "").strip().lower() in ("0", "false", "no"):
        return None
    from app.integrations.opensore.hf_remote import infer_opensore_telemetry_relative

    return infer_opensore_telemetry_relative(raw_alert)


def resolve_opensore_telemetry_dir(raw_alert: dict[str, Any]) -> Path | None:
    """Return the absolute telemetry directory (the ``.../telemetry/YYYY_MM_DD`` folder)."""
    ann = _annotation_dict(raw_alert)
    direct = (
        ann.get("opensore_telemetry_dir")
        or raw_alert.get("opensore_telemetry_dir")
    ...
    if rel and hf_id and not (os.environ.get("OPENSORE_DISABLE_HF_TELEMETRY", "").strip()):
        from app.integrations.opensore.hf_remote import materialize_opensore_telemetry_from_hub

        rev = (
            ann.get("opensore_hf_revision")
            or raw_alert.get("opensore_hf_revision")
            or ann.get("openrca_hf_revision")
            or raw_alert.get("openrca_hf_revision")
            or os.environ.get("OPENSORE_HF_REVISION")
            or None
        )
        try:
...
def inject_opensore_into_resolved_integrations(
    raw_alert: dict[str, Any],
    resolved_integrations: dict[str, Any] | None,
) -> dict[str, Any] | None:
    """When the alert carries opensore paths, inject a Grafana integration with CSV backend.
