...
def _configure_whatsapp() -> tuple[str, str]:
    ...
    account_sid = _prompt_value(
        "Twilio Account SID (starts with AC...)",
        default=_string_value(credentials.get("account_sid")),
    )
    auth_token = _prompt_value(
        "Twilio Auth Token",
        default=_string_value(credentials.get("auth_token")),
    ...
    from_number = _prompt_value(
        "Twilio WhatsApp From number",
        default=_string_value(credentials.get("from_number"), "whatsapp:+14155238886"),
    )
    default_to = _prompt_value(
        "Default recipient phone number (optional)",
        default=_string_value(credentials.get("default_to")),
        allow_empty=True,
    )
    ...
    sync_env_secret("TWILIO_AUTH_TOKEN", auth_token)
    env_path = sync_env_values(
        {
...
def _configure_selected_integrations() -> tuple[list[str], str | None]:
    ...
    integration_choices = [
        ...
        Choice(
            value="whatsapp",
            label="WhatsApp",
            hint="Configure Twilio WhatsApp delivery and personal-agent pairing",
        ),
